...
import { NAV_ITEMS, CHAT_HISTORY } from "@/constants/data";
...
export default function Sidebar({
  open, onToggle, activeNav, onNavChange,
  activeChat, onChatSelect, onNewChat,
}: SidebarProps) {
  const w = open ? 260 : 64;
    ...
    <aside style={{
      ...
      <div style={{ padding: "8px 10px 4px" }}>
        <button onClick={onNewChat} style={{
          width: "100%", background: C.goldGlow,
          ...
          {CHAT_HISTORY.map(h => {
            ...
            return (
              <button key={h.id} onClick={() => onChatSelect(h.id)} style={{
                ...
                border:     active ? `1px solid ${C.border}` : "1px solid transparent",
                borderRadius: 7, cursor: "pointer",
                padding: "7px 10px", textAlign: "left", marginBottom: 2,
              }}>
                <div style={{
                  color:    active ? C.text : C.textMuted,
                ...
                <div style={{ color: C.textDim, fontSize: 10, marginTop: 3 }}>{h.date}</div>
              </button>
            );
          })}
        </div>
        ...
        <button style={{
          ...
          {open && (
            <div style={{ flex: 1, overflow: "hidden", textAlign: "left" }}>
              <div style={{ color: C.text, fontSize: 12, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Lakshya</div>
